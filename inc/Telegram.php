...
if ( file_exists( WPTP_INC . 'TelegramErrorLogger.php' ) ) {
...
class wptpTelegram {
	...
	private function sendAPIRequest( $url, array $content, $post = true ) {
		...
		if ( $result === false ) {
			$result = json_encode(
				[ 'ok' => false, 'error_code' => '-1', 'error' => 'reguest error' ]
			);
		}
...
if ( ! function_exists( 'wptp_curl_file_create' ) ) {
	function wptp_curl_file_create( $filename, $mimetype = '', $postname = '' ) {
